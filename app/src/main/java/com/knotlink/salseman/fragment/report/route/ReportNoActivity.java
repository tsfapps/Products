...
call.enqueue(new Callback<List<ModelRouteNoActivity>>() {
    @Override
    public void onResponse(Call<List<ModelRouteNoActivity>> call, Response<List<ModelRouteNoActivity>> response) {
        List<ModelRouteNoActivity> tModels =response.body();
        pbReportOrder.setVisibility(View.GONE);
        if (tModels.size()>0){
            AdapterRouteNoActivity tAdapter = new AdapterRouteNoActivity(tContext, tModels);
    ...
    @Override
    public void onFailure(Call<List<ModelRouteNoActivity>> call, Throwable t) {
    ...
    private  void callApiShopRequest(){
        ...
        Api api = ApiClients.getApiClients().create(Api.class);
        Call<List<ModelRouteNoActivity>> call = api.viewReportShopNoActivity(shopId,"No Activity", dateFrom, dateTo);
call.enqueue(new Callback<List<ModelRouteNoActivity>>() {
    @Override
    public void onResponse(Call<List<ModelRouteNoActivity>> call, Response<List<ModelRouteNoActivity>> response) {
        List<ModelRouteNoActivity> tModels =response.body();
